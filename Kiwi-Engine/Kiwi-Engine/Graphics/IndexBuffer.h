	...
	public:
		...
		virtual void SetData(std::vector<unsigned long>& bufferData)
		{
			...
			if( m_buffer )
			{
				...
				if( FAILED( hr ) )
				{
					throw Kiwi::Exception( L"IndexBuffer::SetData", L"Failed to map buffer" );
				}
				...
				} else
				{
					...
					throw Kiwi::Exception( L"IndexBuffer::SetData", L"[" + m_name + L"] Index buffer is too small (size: " + Kiwi::ToWString( m_elementCapacity ) + L") to contain passed data (size: " + Kiwi::ToWString( bufferData.size() ) + L")" );
				}
