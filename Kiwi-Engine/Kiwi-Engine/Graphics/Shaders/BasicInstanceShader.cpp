...
namespace Kiwi
{
	...
	BasicInstanceShader::BasicInstanceShader( const std::wstring shaderName, const std::wstring renderer, const std::wstring vsFile, const std::wstring psFile ):
		Kiwi::IShader( shaderName, L"BasicInstanceShader", renderer, vsFile, psFile )
	{
		D3D11_INPUT_ELEMENT_DESC layoutDesc[] =
		{
			...
			/*per-instance world matrix parameters*/
		...
		m_inputLayout = m_renderer->CreateInputLayout( this, layoutDesc, 10 );
	...
	BasicInstanceShader::~BasicInstanceShader()
	{
	}

	void BasicInstanceShader::SetPerFrame( Kiwi::Scene& scene )
	{
		...
		if( pfbBuffer )
		{
			...
			if( FAILED( hr ) )
			{
				MessageBox( NULL, L"Failed to map per frame buffer", L"BasicShaderInst::SetPerFrame", MB_OK );
				return;
			}
	...
	void BasicInstanceShader::SetPerMaterial( Kiwi::Scene& scene, Kiwi::Material& material )
	{
		...
		if( matBuffer )
		{
			...
			if( FAILED( hr ) )
			{
				MessageBox( NULL, L"Failed to map per material buffer", L"BasicShaderInst::SetPerMaterial", MB_OK );
				return;
			}
		...
		{
			throw Kiwi::Exception( L"BasicShaderInst::SetPerMaterial", L"[" + m_shaderName + L"] The pixel object buffer is null" );
		}

	}

	void BasicInstanceShader::SetPerObject( Kiwi::Scene& scene, Kiwi::Viewport& viewport, Kiwi::IModel& model, unsigned int modelSubsetIndex )
	{
		...
		if( vobBuffer )
		{
			...
			if( FAILED( hr ) )
			{
				MessageBox( NULL, L"Failed to map per object buffer", L"BasicShaderInst::SetPerObject", MB_OK );
				return;
			}
			...
			//Copy the new buffer data into the constant buffer
			bufferPtr->viewMat = view;
			bufferPtr->projMat = projection;
		...
		{
			throw Kiwi::Exception( L"BasicShaderInst::SetPerObject", L"[" + m_shaderName + L"] The vertex object buffer is null" );
		}
